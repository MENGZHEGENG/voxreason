...
\begin{itemize}
    ...
    \item empirical diagnostics showing that source-label reconstruction exposes a text-only gap while model-scale and preference variants reveal remaining plan-coverage weaknesses.
\end{itemize}
...
Table~\ref{tab:acoustic-preflight} confirms that the dependency-free feature rows are present and variable. These statistics are not a substitute for a perceptual study or planner-ranking evidence; they only check that future acoustic-control experiments have a usable input channel.
