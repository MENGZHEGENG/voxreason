...
A construct-validity check gives the boundary of this diagnostic. The public source-label split has zero public context-audio rows, two target utterances, two role labels, one scene label, and fifteen emotion-intensity keys. A deterministic train-set lookup keyed only by source emotion and intensity reaches 1.000 exact-plan accuracy on the test split. The current release therefore tests source-record accounting and verifier behavior, not broad dialogue, role, or scene generalization.
