...
A construct-validity check gives the boundary of this diagnostic. The public source-label split has zero public context-audio rows, two target utterances, two role labels, one scene label, and fifteen emotion-intensity keys. All fifteen observed keys map to a single gold plan. A deterministic train-set lookup keyed only by source emotion and intensity reaches 1.000 exact-plan accuracy on the test split, while a leave-key-out stress drops to 0.000 exact-plan accuracy and 0.242 plan-slot accuracy. The public code also builds a source-key-disjoint rerun split with 60 train, 16 development, and 24 test cases and zero train/dev/test key overlap. This split breaks direct key memorization, but it does not by itself separate case-record use from an affect prior such as ``sad'' licensing lower energy or slower rate. Future learned-planner reports need the disjoint split beside a prior-only baseline that receives the source emotion label without the case record. The current release therefore tests source-record accounting and verifier behavior, while the disjoint split is a falsification target rather than completed learned-model ordering evidence.
